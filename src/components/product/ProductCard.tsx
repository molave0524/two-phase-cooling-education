    ...
    <div className={cardClasses}>
      <Link href={`/products/${product.slug}`}>
        ...
        <div className='p-6'>
          ...
          {/* Product Name */}
          <h3 className='font-bold text-lg text-secondary-900 mb-2 line-clamp-2'>{product.name}</h3>

          {/* Product Description */}
          <p className='text-secondary-600 text-sm line-clamp-3 mb-4'>{product.description}</p>
          ...
          <div className='space-y-2 mb-4'>
            {keyFeatures.slice(0, variant === 'featured' ? 4 : 2).map((feature, index) => (
              <div key={index} className='flex items-center gap-2 text-sm'>
                ...
                <span
                  className={`${feature.highlight ? 'font-medium text-secondary-900' : 'text-secondary-600'}`}
                >
                  {feature.text}
                </span>
              </div>
          ...
          <div className='flex items-center justify-between mb-4'>
            ...
            {product.sku && (
              <span className='text-xs text-secondary-500 font-mono'>SKU: {product.sku}</span>
            )}
          </div>
