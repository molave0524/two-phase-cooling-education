...
const SAMPLE_CONVERSATION: ChatMessage[] = [
  ...
  {
    id: '4',
    content: "Excellent question! Two-phase cooling operates at whisper-quiet 18-21 dB under full load - quieter than most traditional systems at idle. No fans needed for the cooling fluid circulation, just a silent pump. You'll actually hear your mechanical keyboard more than the cooling system. Perfect for streaming or late-night gaming sessions.",
    sender: 'assistant',
    timestamp: new Date()
  }
]
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (SAMPLE_CONVERSATION[currentMessageIndex].sender === 'assistant') {
        setIsTyping(true)
        setTimeout(() => {
          setDisplayedMessages(prev => [...prev, SAMPLE_CONVERSATION[currentMessageIndex]])
          setIsTyping(false)
    ...
    <div className="space-y-12">
      ...
      <div className="text-center space-y-4">
        <div className="flex items-center justify-center gap-2">
          ...
          <h2 id="ai-assistant-heading" className="text-3xl font-bold text-secondary-900">
              ...
              {ASSISTANT_CAPABILITIES.map((capability, index) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={`p-3 rounded-technical ${
                      selectedCapability === index ? 'bg-primary-600 text-white' : 'bg-secondary-100 text-secondary-600'
                    }`}>
                      {capability.icon}
                    </div>
                    <div className="flex-1 space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-secondary-600 mt-1">{capability.description}</p>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <ul className="space-y-1">
                          {capability.examples.map((example, exampleIndex) => (
                            <li key={exampleIndex} className="flex items-center gap-2 text-xs text-secondary-600">
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-secondary-600">Trained on comprehensive thermal dynamics and cooling system data</div>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <CheckCircleIcon className="w-5 h-5 text-success-600 mt-0.5 flex-shrink-0" />
                <div>
                  ...
                  <div className="text-sm text-secondary-600">Instant thermal analysis and performance predictions</div>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <CheckCircleIcon className="w-5 h-5 text-success-600 mt-0.5 flex-shrink-0" />
                <div>
                  <div className="font-medium text-secondary-900">Personalized Recommendations</div>
                  <div className="text-sm text-secondary-600">Tailored advice based on your specific hardware and use case</div>
                </div>
              </li>
              <li className="flex items-start gap-3">
                <CheckCircleIcon className="w-5 h-5 text-success-600 mt-0.5 flex-shrink-0" />
                <div>
                  ...
                  <div className="text-sm text-secondary-600">Always ready to help with technical questions and support</div>
                </div>
              </li>
            </ul>
          </div>
