...
export const AIAssistant: React.FC<AIAssistantProps> = ({
    ...
    try {
      ...
      setConversation(prev => ({
        ...
        averageResponseTime: responseTimes.length > 0
          ? [...responseTimes, responseTime].reduce((a, b) => a + b) / (responseTimes.length + 1)
          : responseTime,
      }))
      ...
      setConversation(prev => ({
        ...
        messages: prev.messages.map(msg =>
          msg.id === loadingMessage.id
            ? {
                ...msg,
                content: 'I apologize, but I\'m having trouble responding right now. Please try asking your question again, or browse our FAQ section for common answers.',
                isLoading: false,
                isFallback: true,
                timestamp: new Date(),
              }
  ...
  const rateMessage = useCallback((messageId: string, rating: 'helpful' | 'not_helpful' | 'needs_improvement') => {
    setConversation(prev => ({
      ...prev,
      messages: prev.messages.map(msg =>
        msg.id === messageId
          ? { ...msg, helpfulnessRating: rating }
          : msg
      ),
    }))
    ...
    const feedbackMessages = {
      ...
      needs_improvement: 'Thank you for the feedback. We\'ll use this to enhance our AI assistant.',
    }
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-50 ${className}`}>
      ...
      {/* Chat Interface */}
      {isOpen && (
        <div className={`glass bg-white rounded-equipment shadow-equipment w-96 h-96 flex flex-col ${isMinimized ? 'h-14' : ''}`}>
          {/* Header */}
