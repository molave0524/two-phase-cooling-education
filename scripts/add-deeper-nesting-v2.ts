...
async function addDeeperNesting() {
  try {
    ...
    // Step 1: Create "Custom Water Cooling Kit" bundle
    console.log('1️⃣ Creating Water Cooling Kit bundle...')
    ...
    // Step 2: Get IDs of water cooling components from Extreme PC
    console.log('2️⃣ Finding water cooling components...')
    ...
    // Step 3: Link water cooling components to the kit
    console.log('3️⃣ Linking components to Water Cooling Kit...')
    ...
    // Step 4: Update Extreme PC build to use the kit instead of individual components
    console.log('4️⃣ Updating Extreme PC build to use Water Cooling Kit...')
    ...
    // Step 5: Mark some components as NOT available for individual purchase (only in kit)
    console.log('5️⃣ Setting purchase restrictions using is_available_for_purchase...')
    await db.execute(sql`
      UPDATE products
      SET is_available_for_purchase = false
      WHERE sku IN ('TPC-WCL-TUB-V01', 'TPC-WCL-FIT-V01')
    `)
    console.log('   ✅ Tubing and fittings marked as NOT available for individual purchase\n')
    console.log('   ℹ️  These items can only be purchased as part of the Water Cooling Kit\n')
    ...
    console.log('   └── Water Cooling Kit (Level 1) ✅ Available for purchase')
    console.log('       ├── CPU Block (Level 2) ✅ Available individually')
    console.log('       ├── GPU Block (Level 2) ✅ Available individually')
    console.log('       ├── Radiators (Level 2) ✅ Available individually')
    console.log('       ├── Pump/Res (Level 2) ✅ Available individually')
    console.log('       ├── Tubing (Level 2) ❌ Kit-only (not available for individual purchase)')
    console.log('       ├── Fittings (Level 2) ❌ Kit-only (not available for individual purchase)')
    console.log('       └── Coolant (Level 2) ✅ Available individually')
  } catch (error) {
